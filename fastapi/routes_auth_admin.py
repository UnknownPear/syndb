...
class GoogleGatekeeperPayload(BaseModel):
    token: str

class GatekeeperPasswordPayload(BaseModel):
    password: str

@router.post("/auth/google-gatekeeper")
def google_gatekeeper(payload: GoogleGatekeeperPayload):
    ...
    # Optional allowlist
    allowed = {e.strip().lower() for e in (os.getenv("GATEKEEPER_EMAILS", "").split(",")) if e.strip()}
    allowed_domain = (os.getenv("GATEKEEPER_DOMAIN") or "").lower().strip()
    ...
    access_token = create_access_token({
        "sub": "0",
        "role": "gatekeeper",   # IMPORTANT: your get_current_user_context checks this
        "email": email,
        "name": info.get("name"),
        "roles": ["gatekeeper"],
    })
...
@router.post("/auth/gatekeeper-login")
def gatekeeper_login(payload: GatekeeperPasswordPayload):
    secret_pw = os.getenv("GATEKEEPER_PASSWORD")
    if not secret_pw:
        raise HTTPException(status_code=500, detail="GATEKEEPER_PASSWORD not set")

    if not secrets.compare_digest(payload.password or "", secret_pw):
...
# In routes_auth_admin.py

def get_current_user_context(
    x_user_id: Optional[str] = Header(None, alias="X-User-ID"),
    token_auth: Optional[HTTPAuthorizationCredentials] = Depends(security)
) -> dict:
    ctx = {"id": 0, "roles": [], "is_gatekeeper": False}

    # 1. JWT Check (Production / Gatekeeper)
    if token_auth:
        payload = verify_token(token_auth.credentials)
        if payload:
            if payload.get("role") == "gatekeeper":
                ctx["is_gatekeeper"] = True
                return ctx
            
            ctx["id"] = int(payload.get("sub", 0))
            ctx["roles"] = payload.get("roles", [])
            return ctx

    # 2. Legacy/Local Check
    if x_user_id and str(x_user_id).isdigit():
        ctx["id"] = int(x_user_id)
...
def list_users(
    ...
    # We include 'has_password' so the frontend can show the lock icon.
    sql = [
        """
        SELECT 
            id, name, initials, role, roles, active, avatar_url, 
    ...
    # --- VISIBILITY LOGIC (Gatekeeper / Manager Check) ---
    is_manager = False
    can_see_list = False

    requester_id = ctx["id"]
    is_gatekeeper = ctx["is_gatekeeper"]
    
    # CHECK ENV: Is this Local Dev?
    is_debug = os.getenv("DEBUG", "False").lower() in ("true", "1", "t")

    # 1. Gatekeeper Token (Google Login) -> Can see list
    if is_gatekeeper:
        can_see_list = True
        
    # 2. DEBUG MODE (Local Dev) -> Can see list (Keeps local dev easy)
    elif is_debug:
        can_see_list = True

    # 3. Manager (Already Logged In) -> Can see list
    elif requester_id > 0:
        with db() as (con, cur):
    ...
    # If NOT allowed to see all users:
    if not can_see_list:
        # If logged in as a normal user (Tester/Poster), show ONLY themselves.
        if requester_id > 0:
            sql.append("AND id = %s")
            params.append(requester_id)
        else:
            # Guest in Production (No Gatekeeper token) -> Show Nothing (Empty List)
            return []
    ...
    with db() as (con, cur):
        ...
        # --- REDACT SYSTEM DATA ---
        # Hide internal counters for non-managers.
        # Note: We KEEP 'has_password' so the UI lock icon works.
        if not is_manager:
            for u in users:
