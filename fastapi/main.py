...
async def add_security_headers(request: Request, call_next):
    ...
    # 3. CRITICAL FIX: Allow Google Login Popups to communicate back
    # "same-origin" causes the error you see. "same-origin-allow-popups" fixes it.
    response.headers["Cross-Origin-Opener-Policy"] = "same-origin-allow-popups"
    
    # 4. Content Security Policy (Allow Google Scripts/Frames)
    csp_policy = (
        "default-src 'self'; "
        "img-src 'self' data: https://*.ebay.com https://*.ebayimg.com https://encrypted-tbn0.gstatic.com https://res.cloudinary.com https://lh3.googleusercontent.com; "
        "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://accounts.google.com https://gsi.client-only.google.com https://www.google.com; "
        "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://accounts.google.com; "
        "font-src 'self' https://fonts.gstatic.com; "
        "frame-src 'self' https://accounts.google.com https://gsi.client-only.google.com; "
        "connect-src 'self' https://text.pollinations.ai https://api.ebay.com https://accounts.google.com https://www.googleapis.com;"
    )
